console.log("🚀 ContConfig JS carregando...");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const currentModuleId = app?.dataset.currentModule || "contconfig";
  const currentModuleTitle = app?.dataset.moduleTitle || "ContConfig";
  const currentModuleSubtitle =
    app?.dataset.moduleSubtitle || "Parâmetros e configurações gerais.";
  ...
  async function init() {
    try {
      ...
      console.log("🎉 ContConfig inicializado!");
    } catch (err) {
      console.error("❌ Falha ao inicializar ContConfig:", err);
      goto(LOGIN_PAGE_URL);
    }
  }

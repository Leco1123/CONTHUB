console.log("🚀 ContRelatórios JS carregando...");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function getAccessProfile(user) {
    const profile = normalizeAccessToken(user?.accessProfile || user?.access_profile || user?.role);
  ...
  function normalizeModuleAccess(access) {
    return String(access || "").split("+").map(normalizeAccessToken).filter(Boolean);
  }
  ...
  async function init() {
    try {
      ...
      console.log("🎉 ContRelatórios inicializado!");
    } catch (err) {
      console.error("❌ Falha ao inicializar ContRelatórios:", err);
      goto(LOGIN_PAGE_URL);
    }
  }
